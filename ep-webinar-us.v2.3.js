    ...
    //let phone_regex = /^(\+\d{1,2}\s?)?1?\-?\.?\s?\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}$/gm;
       ...
       pushToEndPoint("https://hooks.zapier.com/hooks/catch/11068981/3hp515j/");
        ...
        $('.profile-analysis-step1').submit();
          ...
          setTimeout(function(){
            location.href = "/upworth-profile-analysis"+window.location.search+"&domain="+domainSelected+"&workexp="+expSelected+"&email="+$('.email').val()+"&fname="+$('.first-name').val()+"&lname="+$('.last-name').val();
          }, 200);
